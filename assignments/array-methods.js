...
// Problem 1
let runsForGigashots = [];
runsForGigashots = runners.filter(function(runner ){
    if (runner.company_name === "Gigashots"){
        return runner;
    }
});
console.log(runsForGigashots);

// Problem 2
let bigDonor = [];
bigDonor = runners.filter(function(runner ){
...
let moreCaps = [];
runners.map(function (runner, index){
    moreCaps[index] = runner.email.toUpperCase();
});
console.log(moreCaps);
